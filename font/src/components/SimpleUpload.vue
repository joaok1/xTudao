...
export default {
  ...
  methods: {
    ...
    async uploadDocument (file) {
      ...
      if (this.previewName !== '' && arrayFileName[arrayFileName.length - 1] === 'pdf') {
        const result = await this.upload(file.raw);
        if (result) {
      ...
      } else if (this.previewName !== '' && arrayFileName[arrayFileName.length - 1] !== 'pdf') {
